    ...
    try {
      ...
      if (logoFileRef.current) {
        ...
        if (file.size > 5 * 1024 * 1024) {
          setErrors({ logo: 'Logo file must be under 5MB. You can also email the logo after submitting.' });
          setLoading(false);
          return;
        }
      ...
      if (!response.ok) {
        ...
        throw new Error(data.error || 'Could not submit sponsorship inquiry.');
      }
      setSubmitted(true);
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              ...
              <div className="border border-gold/15 border-dashed rounded-sm px-4 py-4 bg-fairway hover:border-gold/30 transition-colors">
                ...
                {errors.logo && <p className="text-red-400 font-mono-data text-xs mt-2">{errors.logo}</p>}
              </div>
            ...
            <div className="pt-2">
              {submitError && <p className="text-red-400 font-mono-data text-sm mb-3 text-center">{submitError}</p>}
              <button
                type="submit"
