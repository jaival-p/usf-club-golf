...
export default function SponsorInquiryForm({ selectedPackage, onSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch('/.netlify/functions/sendSponsorshipInquiry', {
        method: 'POST',
      ...
      if (!response.ok) {
        throw new Error(result.error || result.message || 'Failed to send inquiry');
      }
      ...
      <div className="max-w-3xl mx-auto px-6 lg:px-12">
        ...
        >
          <div className="flex items-center gap-4 mb-6">
            ...
            <span className="font-mono-data text-gold text-xs tracking-[0.3em] uppercase">
              Get Involved
            </span>
          </div>
          ...
          <p className="font-inter text-stroke text-lg leading-relaxed">
            Complete the form below and our sponsorship team will follow up with invoice details,
            logo specifications, and next steps.
          </p>
        </motion.div>
        ...
        >
          <div className="mb-8 p-5 border border-gold/20 rounded-sm bg-gold/5 flex items-start gap-3">
            ...
            <p className="font-inter text-stroke text-sm leading-relaxed">
              ...
              Sponsorship payments will be invoiced separately after confirmation from our team.
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div className="grid sm:grid-cols-2 gap-5">
              <div>
                ...
                />
                {errors.companyName && (
                  <p className="text-red-400 font-mono-data text-xs mt-1">
                    {errors.companyName}
                  </p>
                )}
              </div>
            ...
            <div className="pt-2">
              <button
                ...
                className={`w-full py-4 rounded-sm text-sm tracking-widest flex items-center justify-center gap-2 transition-all duration-500 disabled:opacity-80 ${
                  submitted ? 'bg-green-500 text-white font-bold' : 'btn-gold'
                }`}
              >
                {submitted ? (
                  <>
